...
let _status = $state<string>("Connecting...");
let _toastMessage = $state<string | null>(null);
let _toastType = $state<"error" | "success">("error");
...
function handleMessage(event: MessageEvent<IpcMessage>) {
	...
	switch (message.type) {
		case "np3.pong":
			_status = `Connected — ${(message.payload as { status: string }).status}`;
...
<main
	class="min-h-screen bg-(--vscode-editor-background) text-(--vscode-editor-foreground) p-4"
>
