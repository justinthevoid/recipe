<script lang="ts">
	import type { UniversalRecipe } from "$lib/types";
	...
	function initWebGL(glCtx: WebGL2RenderingContext) {
		...
		// Create FBO for multi-pass rendering
	...
	function uploadImage(glCtx: WebGL2RenderingContext, img: HTMLImageElement) {
		...
		// Update canvas size to match aspect ratio
		updateCanvasSize(glCtx, img);

		// Setup FBO texture to match canvas size
	...
	function render(glCtx: WebGL2RenderingContext, sharpness: number) {
		...
		if (needsSharpen && fbo && fboTexture) {
			...
			// Single pass: LUT transform → screen
		...
		if (needsSharpen && sharpenProgram && canvas) {
			...
			glCtx.uniform1f(glCtx.getUniformLocation(sharpenProgram, "u_sharpness"), sharpness / 150.0);
			glCtx.uniform2f(
	...
	async function regenerateLUT() {
		...
		if (pendingGeneration) return; // coalesce — loop will pick up latest
		pendingGeneration = true;

		// Loop until we've rendered the latest generation
		while (true) {
			...
			// If counter advanced during debounce, loop to pick up latest
			if (myGeneration !== generationCounter) continue;

			try {
				const lutData = await generateLUT(JSON.stringify(recipe), LUT_SIZE);

				// If counter advanced during WASM call, loop again
				if (myGeneration !== generationCounter) continue;
				...
				break; // Successfully rendered latest generation
			} catch (err) {
	...
	// Regenerate LUT when recipe changes
	$effect(() => {
		// Access recipe fields to track reactivity
		if (!recipe || !gl || !imageData) return;
		// Touch reactive fields to trigger on any change
		void recipe.exposure;
		void recipe.contrast;
		void recipe.highlights;
		void recipe.shadows;
		void recipe.whites;
		void recipe.blacks;
		void recipe.saturation;
		void recipe.sharpness;
		void recipe.clarity;
		void recipe.red;
		void recipe.orange;
		void recipe.yellow;
...
{#if webgl2Unsupported}
	<div class="flex items-center justify-center h-full bg-[#1e1e1e] text-[var(--vscode-foreground)] rounded-lg">
...
{:else if !imageData}
	<div class="flex flex-col items-center justify-center h-full bg-[#1e1e1e] text-[var(--vscode-foreground)] rounded-lg gap-3">
