...
describe("SchemaDropdown", () => {
	...
	it("protective error state triggers when an invalid value is passed", () => {
		...
		const isDisabled =
			triggerButton.hasAttribute("disabled") || triggerButton.hasAttribute("data-disabled");
	...
	it("resets to defaultValue on double-click", async () => {
		...
		expect(onchange).toHaveBeenCalledWith("pictureControlBase", mockDefinition.defaultValue);
	});
