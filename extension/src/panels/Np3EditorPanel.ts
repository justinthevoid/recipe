...
export class Np3EditorPanel implements vscode.CustomReadonlyEditorProvider {
    ...
    async resolveCustomEditor(
        ...
        try {
            ...
            const errorMessage =
                err instanceof Error ? err.message : "Unknown error starting Go binary";
            ...
            async (message: IpcMessage) => {
                ...
                try {
                    ...
                    this.outputChannel.appendLine(
                        `Go → Webview: ${JSON.stringify(response)}`,
                    );
                    webviewPanel.webview.postMessage(response);
                } catch (err) {
                    const errorMessage =
                        err instanceof Error ? err.message : "IPC communication error";
    ...
    private getHtmlForWebview(webview: vscode.Webview): string {
        ...
        const scriptUri = webview.asWebviewUri(
            vscode.Uri.joinPath(distPath, "webview.js"),
        );
        const styleUri = webview.asWebviewUri(
            vscode.Uri.joinPath(distPath, "webview.css"),
        );
