...
export class BinaryManager {
    ...
    async start(): Promise<void> {
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            this.process.on("error", (err) => {
                ...
                reject(
                    new Error(`Failed to start Go binary: ${err.message}`),
                );
            });

            this.process.on("exit", (code, signal) => {
                this.outputChannel.appendLine(
                    `Go binary exited (code=${code}, signal=${signal})`,
                );
                this.cleanup();
            });
            ...
            if (this.process.stdout) {
                ...
                this.readline.on("line", (line: string) => {
                    try {
                        ...
                        this.outputChannel.appendLine(
                            `Failed to parse Go response: ${line}`,
                        );
                    }
                });
            }
    ...
    private handleResponse(message: IpcMessage): void {
        ...
        if (pending) {
            ...
            this.outputChannel.appendLine(
                `Unsolicited Go message: ${JSON.stringify(message)}`,
            );
        }
    }
